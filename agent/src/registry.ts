 ...
 * total in the registry — the bytes themselves live in each session's own object and
...
export class SessionRegistry extends DurableObject {
  ...
  private ensureSchema() {
    ...
    // writes and deletes files. See `MAX_AGENT_BYTES`.
    this.ctx.storage.sql.exec(
  ...
  addStorageBytes(delta: number): void {
    ...
    this.ctx.storage.sql.exec(
      `INSERT INTO storage (id, bytes) VALUES (1, MAX(?, 0))
       ON CONFLICT(id) DO UPDATE SET bytes = MAX(bytes + excluded.bytes, 0)`,
      Math.trunc(delta)
    );
  }
